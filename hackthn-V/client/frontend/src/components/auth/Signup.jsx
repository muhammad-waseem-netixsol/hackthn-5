...
console.log(signupError, signupLoading, signupSuccess)
    return ( <div>
       ...
       <form className='max-w-[500px] flex gap-3 flex-col mx-auto' onSubmit={onSubmitHandler}>
            <input className='outline-none border py-2 px-2 block w-full' type="text" placeholder='Enter name...' onChange={handleInputChange} value={user.name}  name='name'/>
            ...
            <button  className='border w-full py-2 cursor-pointer hover:bg-slate-100 flex justify-center items-center gap-4' type="submit">SIGN UP {signupLoading && <i className="fa-solid fa-spinner animate-spin"></i>} </button>
            <div className='flex justify-center items-center gap-5'><hr className='w-[60px]' />OR <hr className='w-[60px]' /></div>
            <button  className='border w-full py-2 cursor-pointer hover:bg-slate-100 flex justify-center items-center gap-4' type="submit"><Link to={`/login`}>LOG IN</Link></button>
       </form>
    </div>
    );
}
